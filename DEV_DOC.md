...
Containers: mariadb (database), wordpress (WordPress + PHP-FPM), nginx (only entrypoint, port 443).
...
Connect to MariaDB: docker exec -it mariadb mariadb -u user -p wordpress (password from secrets/db_password.txt)

Shell in a container: docker exec -it mariadb bash (or wordpress, nginx)

View logs: make logs or docker compose -f srcs/docker-compose.yml logs <service>
